...
func (r *ReceivedMessage) put(db *sql.DB) error {
	...
	lastInsertId := 0
	err = stmt.QueryRow(r.ChatID, r.MessageHash, r.MessageID, r.ReceiverKeyUID, r.PeerID, r.NodeName, r.SentAt, r.Topic, r.MessageType, r.MessageSize, r.CreatedAt, r.PubsubTopic, r.StatusVersion, r.DeviceType).Scan(&lastInsertId)
	if err != nil {
		return err
	}
	r.ID = lastInsertId
